...
import service.ServiceApiImplDB;
...
public class App {
    ...
    public static final String CLIENTS = "Display clients";
    ...
    private JPanel dispClients;
    private JScrollPane oldPanel;
    private JButton oldDelete;
    private JPanel oldSpecPanel;

    public App(){

        mainPanel.setLayout(new CardLayout());
        serviceAPI = new ServiceApiImplDB();
        ...
        setDisplayClientsButtonDetails();
    ...
    private void setShowToPayButtonDetails() {
        ...
        showToPayButton.addActionListener(ev ->{
            JOptionPane.showMessageDialog(mainFrame, "You have to pay " + serviceAPI.getTotalToPay(selectedSeats),
                    "Total to pay",
    ...
    private void createButtonPanel() {
        ...
        constraints.gridy = 2;
        buttons.add(displayClientsButton,constraints);
        constraints.gridy = 3;
    ...
    private void setDisplayClientsButtonDetails(){
        displayClientsButton = new JButton("Show all clients");
        displayClientsButton.setVerticalAlignment(SwingConstants.CENTER);
        displayClientsButton.setPreferredSize(new Dimension(300, 50));
    ...
    private void createDisplayClientsPanel(){
        int clientsSize = serviceAPI.displayClients().size();
        List<String> names = serviceAPI.displayClients()
                .stream()
                .map(e -> e.getName())
                .collect(Collectors.toList());

        dispClients = new JPanel(new GridBagLayout());
        GridBagConstraints dispConstraints = new GridBagConstraints();
        dispConstraints.anchor = GridBagConstraints.CENTER;
        dispConstraints.gridy = 1;
        dispConstraints.weighty = 1;
        dispConstraints.weightx = 1;
        dispConstraints.fill = GridBagConstraints.BOTH;

        JPanel clients = new JPanel(new GridBagLayout());
        GridBagConstraints constraints = new GridBagConstraints();
        constraints.anchor = GridBagConstraints.CENTER;

        ButtonGroup clientsButtons = new ButtonGroup();
        List<JToggleButton> buttonsList = new ArrayList<>();
        AtomicInteger gridy = new AtomicInteger(1);
        for(int i = 0; i < names.size(); i++){
            String e = names.get(i);
            constraints.gridy = gridy.get();
            JToggleButton client = new JToggleButton(e);
            client.setMargin(new Insets(20, 10, 20, 10));
            client.setPreferredSize(new Dimension(300, 50));
            client.setActionCommand(e);
            client.setBackground(Color.LIGHT_GRAY);
            client.addItemListener(ev->{
            ...
            gridy.getAndIncrement();
            clientsButtons.add(client);
            clients.add(client, constraints);
            buttonsList.add(client);
        }
        clients.setVisible(true);
        JScrollPane pane = new JScrollPane(clients);
        oldPanel = pane;
        dispClients.add(pane, dispConstraints);
        dispConstraints = new GridBagConstraints();
        dispConstraints.gridy = 2;


        JButton deleteClient = new JButton("Delete client");
        deleteClient.setMargin(new Insets(10, 10, 10, 10));
        deleteClient.addActionListener(ev->{
            try{
                int count = (int) buttonsList.stream().filter(e -> e.getModel().isSelected()).count();
                if(count == 0){
                    throw new NoNameException("No name");
                }
                serviceAPI.deleteClient(clientsButtons.getSelection().getActionCommand());
                CardLayout cl = (CardLayout) mainPanel.getLayout();
                cl.show(mainPanel, ACTIONS);
        ...
        oldDelete = deleteClient;
        dispClients.add(deleteClient, dispConstraints);

        dispClients.setName(CLIENTS);
        dispClients.addAncestorListener(new AncestorListener() {
            @Override
        ...
        mainPanel.add(dispClients, CLIENTS);
    }
    ...
    private void refreshClients() {
        ...
        deleteClient.addActionListener(ev->{
            try{
                ...
                JLabel jl = new JLabel("Enter the admin password: ");
                Box box = Box.createHorizontalBox();
                box.add(jl);
                box.add(jpf);
                int pass = JOptionPane.showConfirmDialog(null, box, "Admin checker", JOptionPane.OK_CANCEL_OPTION);
                ...
                if(pass == JOptionPane.OK_OPTION){
                    ...
                    if(sb.toString().equals("123456789")){
                        JOptionPane.showMessageDialog(mainFrame, "Deleted client", "Success", JOptionPane.INFORMATION_MESSAGE);
                    }
                    else{
                        JOptionPane.showMessageDialog(mainFrame, "Wrong Password", "Error", JOptionPane.ERROR_MESSAGE);
                    }
                }
                ...
                JOptionPane.showMessageDialog(mainFrame, "No Client selected", "Error", JOptionPane.ERROR_MESSAGE);

            }
        });
        dispClients.remove(oldDelete);
        dispClients.add(deleteClient, dispConstraints);
        oldDelete = deleteClient;
        dispClients.revalidate();
        dispClients.repaint();
    ...
    private void createSpectaclePanel() {
        ...
        spectaclePanel.addAncestorListener(new AncestorListener() {
            @Override
            public void ancestorAdded(AncestorEvent event) {
                refreshSpectacles();
            }
        ...
        oldSpecPanel = spectaclePanel;
        mainPanel.add(spectaclePanel, SPECTACLES);
    }

    private void refreshSpectacles() {
        int numOfSpecs = serviceAPI.numberOfSpectacles();
        List<String> specsNum = serviceAPI.getSpectaclesName();
        ...
        for(int i = 0; i < numOfSpecs; i++){
            ...
            button.setAction(spectacleAction(i));
            spectaclePanel.add(button);
            System.out.println(specsNum.get(i));
            button.setText(specsNum.get(i));
        }
    ...
    private AbstractAction spectacleAction(int index) {
        return new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                Spectacle spectacle = serviceAPI.getSelectedSpectacle(index+1);
                List<Seat> seatList = serviceAPI.getSpectacleSeats(index+1);
